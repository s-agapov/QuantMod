...
class TradingSystem(abc.ABC):
    ...
    def write_states(self):
        state_fn = 'States/' + self.robot_name + '.txt'
        robot_state = pd.read_csv(state_fn)
        ...
        if robot_state.shape[0] == 0: 
            ...
            robot_state.loc[0, 'MP'] = self.market_position
            robot_state.loc[0, 'OP'] = self.open_position
            robot_state.loc[0, 'num_shares'] = self.num_shares

            robot_state.to_csv(state_fn, index = False)
        return
