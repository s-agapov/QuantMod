...
#    exit short MOC when close is $3000 above entry or when SMA crosses above IMA
#--------------------------------------------------------------------------------
 ...
 #Long Exit - 2 avg1 crosses below avg2
        if mp >= 1 and avg1 < avg2 and barsSinceEntry > 1:
            price = myClose[i]
            tradeName = "Lavg1<avg2"
            exitDate =myDate[i]
            numShares = curShares
            exitQuant.append(numShares)
            profit,trades,curShares = exitPos(price,myDate[i],tradeName,numShares)
...
#Short Exit - 2 avg1 crosses above avg2            
        if mp <= -1 and avg1 > avg2 and barsSinceEntry > 1:         
            price = myClose[i]
            tradeName = "S-avg1>avg2"
            exitDate = myDate[i]
            numShares = curShares
            exitQuant.append(numShares)
            profit,trades,curShares = exitPos(price,myDate[i],tradeName,numShares)
