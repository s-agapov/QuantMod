...
for marketCnt in range(0,numMarkets):
    ...
    if isStockData:
       myBPV = 100 # 100 shares
       myMinMove = 0.01 # penny move
...
#--------------------------------------------------------------------------------
 #Long Entry Logic
        if (mp != 1) and myClose[i] >= keltUpChan:
            profit = 0
            price = myClose[i]
            if mp <= -1:
...
# Short Entry Logic
        if (mp !=-1)  and myClose[i] < keltDnChan:
            profit = 0
            price = myClose[i]
            if mp >= 1:
 ...
 #Long Exit - 1 Fixed Dollar Stop Loss
        ...
        if mp >= 1 and myClose[i] < keltAvg and barsSinceEntry > 1:
            price = myClose[i]
            tradeName = "LX-C<KeltAvg"
            exitDate =myDate[i]
            numShares = curShares
            exitQuant.append(numShares)
            profit,trades,curShares = exitPos(price,myDate[i],tradeName,numShares)
