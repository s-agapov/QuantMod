...
sysName = 'BBSysWFilter' #System Name here
...
#--------------------------------------------------------------------------------
 #Long Entry Logic
        if (mp != 1) and myHigh[i] >= buyLevel and \
           (buyLevel - exitLevel)*myBPV < 3000:
            profit = 0
...
# Short Logic
        if (mp != -1) and myLow[i] <= shortLevel and \
            (exitLevel - shortLevel)*myBPV < 3000 :
            profit = 0
